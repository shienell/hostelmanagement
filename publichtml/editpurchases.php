...
class editpurchases
{
function editpurchases($params,$tpl)
{
...
echo "<html>";
			echo "<body bgcolor=#70A3D0><form name=f3 method=POST action=index.php?service=CHANGEPURCHASES><center>
			<h2><u>UPDATE PURCHASES FOR $h HOSTEL FOR $month MONTH AND $year YEAR ";
			
			echo "<br><table height=100 width=50 border=1>";
			
	$rs=$conn->execute("select * from purchases where hostelname='$hostelname' and year=$year and month='$month' order by sno");
...
echo "</tr>";
	...
	for($i=1;$i<=$cnt->fields[0];$i++)
	{
		...
		$rs->MoveNext();
			...
			echo "</table></CENTER>
			<center><br><br><br><br>
			<B><input type=submit value=UPDATE><br><br></B>
			</center>";	
		echo "</form>
			</body>
			</html>";
