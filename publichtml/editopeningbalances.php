<?php
class editopeningbalances
{
function editopeningbalances($params,$tpl)
{
...
echo "<html>";
			echo "<body bgcolor=#70A3D0><form name=f3 method=POST action=index.php?service=CHANGEOPENINGBALANCES><center>
			<h2><u>UPDATE OPENING BALANCE FOR $h HOSTEL FOR $month MONTH AND $year YEAR ";
			
			echo "<br><table height=100 width=50 border=1>";
			
	$rs=$conn->execute("select * from openingbalance where hostelname='$hostelname' and year=$year and month='$month' order by sno");
