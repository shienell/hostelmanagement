<?php
class editclosingbalances
{
function editclosingbalances($params,$tpl)
{
...
$sql=$conn->execute("select * from closingbalances where hostelname='$hostelname' and month='$month' and year=$year");
if($sql)
{
echo "<html>";
			echo "<body bgcolor=#70A3D0><form name=f3 method=POST action=index.php?service=CHANGECLOSINGBALANCES><center>
			<h2><u>UPDATE CLOSING BALANCES FOR $h HOSTEL FOR $month MONTH AND $year YEAR ";
			
			echo "<br><table height=100 width=50 border=1>";
			
	$rs=$conn->execute("select * from closingbalances where hostelname='$hostelname' and year=$year and month='$month' order by sno");
...
echo "<th colspan=3>Opening Balance</th>";
...
echo "</tr>";
	...
	for($i=1;$i<=$cnt->fields[0];$i++)
	{
		...
		echo "</form>
			...
			{
			echo "CLOSING BALANCES DOES NOT EXIST";
			}
}
}
